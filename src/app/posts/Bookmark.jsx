...
const Bookmark = ({ postId, bookmarked }) => {
    ...
    return (
        <button className='rounded-full duration-500 hover:bg-gray-200 p-3 focus:bg-none' onClick={() => handleBookmark(postId)}>
            <BsBookmarkFill className={`transition-shadow ${bookmarked ? 'text-blue-500' : 'stroke-gray-500 fill-white stroke-1'} ${mutation.isLoading ? 'animate-pulse cursor-progress' : 'animate-none'}`} />
        </button>
    )
}
