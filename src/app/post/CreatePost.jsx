...
const CreatePost = () => {
  ...
  useEffect(() => {
    if (data && pathname.startsWith('/post/edit')) {
      const { title, description, price, category, isNetting, image } = data;
      setState({ title, description, price, category, isNetting, image });
  ...
  useEffect(() => {
    if (pathname.startsWith('/post/edit') && error?.response?.status === 404) {
