...
import { validationCreatePost } from './FormCreateValidation';
import FormCreateAd from './FormCreateAd';
import SelectCategory from './SelectCategory';
import { useLocation, useNavigate, useParams } from 'react-router';
import { useUpdateAd, useAd } from '../hooks/api/useAdsApi';
...
const CreateAd = () => {
  ...
  const { isFetching, data, error } = useAd(slug, hashId)

  useEffect(() => {
    if (data && pathname.startsWith('/ads/edit')) {
  ...
  useEffect(() => {
    if (pathname.startsWith('/ads/edit') && error?.response?.status === 404) {
  ...
  return (
    <section className={`container mx-auto p-3 font-light`}>
      <div className='md:w-2/3 mx-auto'>
        ...
        {stepForm === 3 && <FormCreateAd formik={formik} isNetting={post?.isNetting} pre={pre} updatePost={handleUpdate} id={data?._id}/>}
      </div>
    </section>
  );
}
